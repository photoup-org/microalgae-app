...
export const config = {
    matcher: [
         ...
         *
         * api/webhooks and api/telemetry are called by the edge worker, which has a
         * bearer token but no session cookie. Left in the matcher they would be
         * redirected to /auth/login, so the worker would see a redirect instead of
         * its endpoint and retry into its dead-letter queue. They authenticate
         * themselves against EDGE_WEBHOOK_SECRET instead.
         */
        "/((?!api/webhooks|api/telemetry|_next/static|_next/image|favicon.ico).*)",
    ],
};
